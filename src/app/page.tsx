...
import WhatsAppWelcome, { WhatsAppGlyph } from '@/components/WhatsAppWelcome';
import './landing.css';
...
export default function LandingPage() {
  ...
  return (
    <div className={`cnmp-root${menuOpen ? ' menu-open' : ''}${waBarVisible ? ' has-wa-bar' : ''}`}>
      ...
      {/* WhatsApp float — CTA de compra asistida */}
      <a
        className="wa wa-cta"
        href={waLink(t('waBanner.buyMessage'))}
        target="_blank"
        rel="noopener noreferrer"
        aria-label={t('waBanner.fabAria')}
      >
        <span className="fab-t">{t('waBanner.fabLabel')}</span>
        <WhatsAppGlyph />
      </a>
