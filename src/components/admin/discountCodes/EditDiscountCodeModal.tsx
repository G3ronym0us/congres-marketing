...
export default function EditDiscountCodeModal({ discountCode, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(Array.isArray(msg) ? msg.join(', ') : (msg || 'Error al actualizar el código'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, background: PANEL, border: `1px solid ${LINE}`, borderRadius: 20, padding: '28px 28px 24px', width: '100%', maxWidth: 460, boxShadow: '0 32px 80px rgba(0,0,0,.6)' }}>
        ...
        <div style={{ background: PANEL2, border: `1px solid ${LINE}`, borderRadius: 10, padding: '12px 14px', marginBottom: 18, display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ fontFamily: 'Oxanium, sans-serif', fontWeight: 800, fontSize: 16, color: NEON, letterSpacing: '.05em' }}>
            {discountCode.code}
          </div>
          <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 12, color: MUTE }}>
            Usos actuales: <span style={{ color: '#fff' }}>{discountCode.currentUses}</span>
            {' · '}Creado: <span style={{ color: '#fff' }}>{formatDate(discountCode.createdAt)}</span>
          </div>
        </div>
