...
const EditTestimonialModal: React.FC<Props> = ({ isOpen, onClose, testimonial, onSuccess }) => {
  ...
  const onSubmit = async (data: UpdateTestimonialInput) => {
    try {
      ...
      Swal.fire({ icon: 'error', title: 'Error', text: error.response?.data?.message || 'No se pudo actualizar el testimonio' });
    }
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, background: PANEL, border: `1px solid ${LINE}`, borderRadius: 20, padding: '28px 28px 24px', width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 32px 80px rgba(0,0,0,.6)' }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <Field label="Apellido *" error={errors.lastName?.message}>
              <Controller name="lastName" control={control}
                rules={{ required: 'Requerido', minLength: { value: 2, message: 'Mínimo 2 caracteres' }, maxLength: { value: 50, message: 'Máximo 50 caracteres' } }}
                render={({ field }) => <input {...field} style={{ ...iS, borderColor: errors.lastName ? '#ff6b6b' : LINE2 }} />} />
            </Field>
          </div>
          ...
          <Field label="Cargo / Posición *" error={errors.position?.message}>
            <Controller name="position" control={control}
              ...
              render={({ field }) => <input {...field} style={{ ...iS, borderColor: errors.position ? '#ff6b6b' : LINE2 }} />} />
          </Field>
